% 3D RRT vs APF-RRT 对比（APF仅在XY平面作用，后处理：高度平滑+XY剪枝）
clear; close all; clc;
rng('shuffle');
...
% 后处理开关（仅对APF-RRT）
enable_postprocess = true; % 是否启用高度平滑+剪枝
...
fprintf('开始运行 %d 次仿真（APF仅在XY平面，后处理：平滑+剪枝）...\n', num_trials);
...
for trial = 1:num_trials
    ...
    try
        [path, nodes, parent, stats, ok] = runRRT(start, goal, obstacles_inflated, ...
            ...
            enable_postprocess, prune_max_passes);
        if ok
...
figure('Name', '路径示例对比（APF XY+后处理）', 'Position', [100, 100, 800, 600]);
hold on; grid on; axis equal; view(3);
xlabel('X'); ylabel('Y'); zlabel('Z');
title(sprintf('RRT vs APF-RRT (XY+平滑+剪枝) 成功率 RRT:%.1f%% APF:%.1f%%', ...
...
figure('Name', '统计指标独立对比（APF XY+后处理）', 'Position', [200, 200, 1200, 800]);
for i = 1:6
    subplot(2,3,i);
...
sgtitle(sprintf('20次独立仿真（APF XY+平滑+剪枝）– 各指标均值与标准差 (成功率 RRT:%.1f%% APF:%.1f%%)', ...
    100*sum(success_rrt)/num_trials, 100*sum(success_apf)/num_trials));
...
function [final_path, nodes, parent, stats, success] = runRRT(start, goal, obstacles_inflated, ...
    ...
    final_path = original_path;   % 默认

    % ---- APF 后处理：高度平滑 + 剪枝 ----
    if use_apf && enable_postprocess
        % 1. 高度平滑
        final_path = smoothPathHeight(final_path, obstacles_inflated, start, goal);
        % 2. 剪枝（XY平面，但碰撞检测是三维的）
        final_path = iterativePrune(final_path, obstacles_inflated, prune_max_passes);
    end

    % ---- 统计指标 ----
    stats = computeRawStats(final_path);
    stats.iter = iter;
    stats.nodes = size(nodes,1);
    stats.path_nodes = size(final_path,1);
    success = true;
end

% ========== 高度平滑 ==========
function path_smoothed = smoothPathHeight(path, obstacles, start, goal)
    if size(path,1) < 2
        path_smoothed = path;
        return;
    end
    horiz = zeros(size(path,1),1);
    for i = 2:size(path,1)
        dx = path(i,1) - path(i-1,1);
        dy = path(i,2) - path(i-1,2);
        horiz(i) = horiz(i-1) + sqrt(dx^2 + dy^2);
    end
    total_horiz = horiz(end);
    if total_horiz < 1e-6
        path_smoothed = path;
        return;
    end
    desired_z = start(3) + (goal(3) - start(3)) * (horiz / total_horiz);
    new_path = path;
    new_path(:,3) = desired_z;
    % 检查碰撞
    is_collision_free = true;
    for i = 1:size(new_path,1)-1
        if checkCollision(new_path(i,:), new_path(i+1,:), obstacles)
            is_collision_free = false;
            break;
        end
    end
    if is_collision_free
        path_smoothed = new_path;
    else
        path_smoothed = path;
    end
end
